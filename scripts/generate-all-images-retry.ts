...
async function run() {
  ...
  console.log(`Found ${products.length} total products to process in target categories.`);
  ...
  for (const p of products) {
    ...
    if (hasPlaceholder || productImages.length === 0) {
      count++;
      console.log(`\n[${count}] Generating image for ${p.name}...`);
      ...
      while (!success && retries < maxRetries) {
        ...
        try {
          ...
          if (!res.ok) {
             console.log(`  Retry ${retries+1}: Failed with status ${res.status}`);
             retries++;
             await new Promise(r => setTimeout(r, 6000 + (retries * 2000))); // Backoff
             continue;
          }
          ...
          console.error(`  -> Retry ${retries+1}: Error on ${p.name}: ${err.message}`);
          retries++;
          await new Promise(r => setTimeout(r, 6000 + (retries * 2000))); // Backoff
        }
      }
      ...
      if (!success) {
        console.log(`  -> FAILED completely for ${p.name}`);
      }
