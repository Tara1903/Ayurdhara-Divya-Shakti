...
exports.SYNONYM_DICTIONARY = void 0;
exports.extractCleanIngredient = extractCleanIngredient;
exports.determineProductForm = determineProductForm;
exports.analyzeProductForImages = analyzeProductForImages;
// Common Hindi <-> English botanical & ingredient dictionary
exports.SYNONYM_DICTIONARY = {
    'haldi': 'turmeric',
    'turmeric': 'turmeric',
    'saunf': 'fennel',
    'fennel': 'fennel',
 ...
 */
function extractCleanIngredient(name) {
    var cleaned = name.toLowerCase();
    // Replace slashes and symbols with spaces
    cleaned = cleaned.replace(/[/\\(),-]/g, ' ');
    var tokens = cleaned.split(/\s+/).filter(function (t) { return t.trim().length > 0; });
    var filtered = tokens.filter(function (t) { return !STOP_WORDS.includes(t); });
    return filtered.join(' ').trim() || name.toLowerCase().trim();
}
/**
 * Determines exact product physical form based on Category and Name
 */
function determineProductForm(name, category, subcategory) {
    ...
    if (lowerCat.includes('single herb') ||
    ...
    // 3. Herbal Teas
    if (lowerCat.includes('tea') ||
        lowerSub.includes('tea') ||
        lowerName.includes('tea')) {
        return 'tea';
    }
    // 4. Kadha
    if (lowerCat.includes('kadha') ||
        lowerSub.includes('kadha') ||
        lowerName.includes('kadha')) {
        return 'kadha';
    }
    // 5. Wellness Drinks
    if (lowerCat.includes('drink') ||
 ...
 */
function analyzeProductForImages(name, category, subcategory) {
    var e_1, _a;
    if (category === void 0) { category = ''; }
    if (subcategory === void 0) { subcategory = ''; }
    var clean = extractCleanIngredient(name);
    var form = determineProductForm(name, category, subcategory);
    var validationNotes = [];
    var formDescription = '';
    var keywordSuffix = '';
    var confidenceScore = 95;
    // Resolve botanical / ingredient synonyms
    var resolvedIngredient = clean;
    try {
        for (var _b = __values(Object.entries(exports.SYNONYM_DICTIONARY)), _c = _b.next(); !_c.done; _c = _b.next()) {
            var _d = __read(_c.value, 2), key = _d[0], val = _d[1];
            if (clean.includes(key)) {
                resolvedIngredient = "".concat(clean, " ").concat(val);
                break;
            }
        }
    }
    ...
    var generatedKeywords = "".concat(clean, " ").concat(keywordSuffix).replace(/\s+/g, ' ').trim();
    var aiPrompt = "Award-winning commercial product photography of ".concat(clean, " (").concat(resolvedIngredient, "). The shot highlights ").concat(formDescription, ". Studio lighting, soft shadows, pristine minimalist light cream natural background, luxury organic wellness brand aesthetic, centered composition, ultra-sharp focus, 8k resolution, no text, no logos, no watermarks, no packaging labels, photorealistic masterpiece.");
    return {
        originalName: name,
        category: category,
